...
public class Dcm2Jpg {
    ...
    public void initImageWriter(String formatName, String suffix,
        ...
        Optional<ImageWriter> optionalImageWriter = StreamSupport.stream(iterable.spliterator(), false)
                .filter(matchClassName(clazz))
                .findFirst();

        if (optionalImageWriter.isPresent()) {
            imageWriter = optionalImageWriter.get();
        } else {
            throw new IllegalArgumentException(
