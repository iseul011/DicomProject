...
//---- 썸네일 ----//
let isDivVisible = false; // 초기값은 숨겨진 상태
...
async function thumbnailBox() {
    ...
    if (isDivVisible) {
        ...
        seriesTabs.forEach((seriesTab, index) => {
            ...
            imgElement.style.marginBottom = '10px'; //원하는 마진 값으로 조정

            // 시리즈 번호 엘리먼트 생성
            const seriesNumberElement = document.createElement('div');
            seriesNumberElement.classList.add('bm');
            seriesNumberElement.textContent = `시리즈 ${seriesNumber}`;

            // fragment에 추가
            fragment.appendChild(imgElement);
            fragment.appendChild(seriesNumberElement);
        });
